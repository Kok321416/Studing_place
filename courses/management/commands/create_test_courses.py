    ...
    def handle(self, *args, **options):
        ...
        for i, course_data in enumerate(test_courses[:count]):
            ...
            if Course.objects.filter(title=course_data["title"]).exists():
                self.stdout.write(
                    self.style.WARNING(
                        f'Курс "{course_data["title"]}" уже существует, пропускаем'
                    )
                )
                continue
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Успешно создано {created_courses} тестовых курсов!")
        )
        ...
        for course in Course.objects.filter(owner=test_user).order_by("-created_at"):
            ...
            self.stdout.write(
                f"• {course.title} - {price_info} ({lessons_count} уроков)"
            )
