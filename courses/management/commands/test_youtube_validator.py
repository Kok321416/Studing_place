...
class Command(BaseCommand):
    ...
    def test_single_url(self, url):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ URL прошел валидацию: {url}"))
        except ValidationError as e:
            self.stdout.write(self.style.ERROR(f"❌ URL отклонен: {e}"))
    ...
    def test_all_examples(self):
        ...
        for url in valid_urls:
            try:
                ...
                self.stdout.write(self.style.ERROR(f"  ❌ ОШИБКА: {url} - {e}"))
        ...
        for url in invalid_urls:
            try:
                validate_youtube_url(url)
                self.stdout.write(self.style.ERROR(f"  ⚠️  ПРОПУЩЕН: {url}"))
            except ValidationError as e:
