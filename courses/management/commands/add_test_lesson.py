...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not user:
            self.stdout.write(
                self.style.ERROR('Нет пользователей в системе. Сначала создайте пользователя.')
            )
            return
        ...
        self.stdout.write(f'Курсы с оплатой: http://localhost:8000/courses-with-payment/')
